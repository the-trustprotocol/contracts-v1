...
contract UserFactory is IUserFactory, IUserFactorySwapables, Ownable2StepUpgradeable, UUPSUpgradeable {
  ...
  function createUserWithBond(address user1,address user2,uint256 initialAmount,address bondFactoryAddress,address yieldProviderService) public payable returns (address){
    ...
    if(initialAmount > 0 ){
      ...
      protocolUser1.createBond(user2, yieldProviderServiceContract.depositToken(), yieldProviderService, initialAmount,bondFactoryAddress);
    }
  ...
  function createUserOnBehalf(address user) private returns (address,bool) {
    ...
    if(registry.addressToUserContracts(user) == address(0)){
      IUser protocolUser = new User(address(this),address(identityRegistry), address(userSettings),address(this));
